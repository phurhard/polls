...
async function testDatabaseConnection() {
  ...
  try {
      ...
      try {
        ...
        if (tableError) {
          if (tableError.code === 'PGRST116') {
            ...
            logWarning(`Table '${table}' exists but has permission issues: ${tableError.message}`)
          }
        } else {
    ...
    try {
      ...
      const { data: funcData, error: funcError } = await supabase.rpc('user_has_voted', {
      ...
      if (funcError) {
        if (funcError.code === '42883') {
          ...
          logWarning(`Functions exist but returned error: ${funcError.message}`)
        }
      } else {
    ...
    try {
      ...
      if (pollsError) {
        if (pollsError.code === '42501') {
          logWarning('RLS policies are very restrictive (might need adjustment)')
        } else {
    ...
    console.log('='.repeat(60))
    logInfo(`Supabase Dashboard: ${supabaseUrl.replace('/rest/v1', '')}/project/default`)
...
testDatabaseConnection()
  .then((success) => {
    if (success) {
      ...
      console.log('\n' + colors.red + '💥 Some tests failed. Please check your setup.' + colors.reset)
      process.exit(1)
    }
  })
  .catch((error) => {
