...
export function PollCard({
  ...
  const totalVotes = 'totalVotes' in poll ? poll.totalVotes : poll._count?.votes || 0
  ...
  const handleDeleteClick = () => {
    if (onDelete && window.confirm("Are you sure you want to delete this poll?")) {
      onDelete(poll.id)
    }
  }
        ...
        <div className="space-y-4">
            ...
            {poll.options.slice(0, 3).map((option) => {
              ...
              return (
                <div key={option.id} className="flex items-center justify-between text-sm">
                  <span className="truncate flex-1 pr-2">{option.text}</span>
          ...
          {showActions && (
            <div className="flex gap-2 pt-2">
              ...
              >
                {isLoading ? "Loading..." : hasUserVoted ? "View Results" : "Vote"}
              </Button>
